...
class LibSqlMemoryServer {
  ...
  public static async create(): Promise<LibSqlMemoryServer> {
    ...
    instance.db = await DatabaseManager.getInstance(config);
        ...
        try {
          switch (request.params.name) {
            case "create_entities": {
              ...
              await this.db.createEntities(entities);
              return {
                content: [
                  {
                    type: "text",
            ...
            case "search_nodes": {
              ...
              const result = await this.db.searchNodes(query);
              return {
                content: [
                  {
                    type: "text",
            ...
            case "read_graph": {
              const result = await this.db.readGraph();
              return {
                content: [
                  {
                    type: "text",
            ...
            case "create_relations": {
              ...
              await this.db.createRelations(internalRelations);
              return {
                content: [
                  {
                    type: "text",
